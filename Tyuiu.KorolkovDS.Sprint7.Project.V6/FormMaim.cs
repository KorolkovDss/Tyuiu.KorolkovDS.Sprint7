...
namespace Tyuiu.KorolkovDS.Sprint7.Project.V6
{
    public partial class FormMain_KDS : Form
    {
        ...
        {
            MessageBox.Show($"Кнопка 'Добавить' добавляет данные пациаента которые вы ввели {Environment.NewLine}" +
                $"Кнопка 'Удалить' удаляет выбранного вами пациента{Environment.NewLine}" +
                $"Кнопка 'Изменить' изменяет данные выбранного пациента, предварительно введите данные которые нужно изменить в поля ввода, те которые вы не хотите изменять заполните данными которые уже есть у пациента{Environment.NewLine}" +
                $"Кнопка 'Сохранить' сохраняет данные ранее добавленных пациентов в файл формата .CSV{Environment.NewLine}" +
                $"Кнопка 'Открыть' открывает папку с возможность выбора нужного вам файла формата .CSV ", "Руководство пользователя", MessageBoxButtons.OK);
        }
    }
}
